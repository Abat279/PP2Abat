...
_rnd.seed(42)
# Stars generated lazily based on screen size — see draw_bg
_STARS_CACHE = {}
...
def _get_stars(W, H):
    ...
    if key not in _STARS_CACHE:
        _rnd.seed(42)
        _STARS_CACHE[key] = [
            (_rnd.randint(0, W - 1), _rnd.randint(0, H - 1), _rnd.randint(1, 3))
            for _ in range(80)
        ]
        ...
        for ev in pygame.event.get():
            ...
            if ev.type == MOUSEBUTTONDOWN:
                ...
                for i, (label, key) in enumerate(buttons):
                    bx = W//2 - bw//2
                    by = start_y + i*(bh+gap)
                    if pygame.Rect(bx, by, bw, bh).collidepoint(mx2, my2):
                        return key
        for i, (label, key) in enumerate(buttons):
            bx = W//2 - bw//2
            by = start_y + i*(bh+gap)
            ...
            draw_button(screen, label, (bx, by, bw, bh), hover, col)
        pygame.display.flip()
        clock.tick(60)
...
def leaderboard_screen(screen, clock):
    ...
    while True:
        ...
        for i, entry in enumerate(board[:10]):
            ...
            screen.blit(rf.render(f'{i+1}.', True, col),                          (W//2-240, ry))
